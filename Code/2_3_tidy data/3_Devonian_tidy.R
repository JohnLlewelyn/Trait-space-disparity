...
#new_env1 <- new.env()
DF <- readRDS(paste(filepath,"/output/Mig&GoImp_2025.RDS", sep=""))

#subset to communities/sites to keep
kp <- c("Miguasha","Gogo","Canowindra")
mg <- DF
mg <- mg[mg$site%in%kp,]
...
#add some columns
mg$habitat = ifelse(mg$community=="Miguasha","estuary", ifelse(mg$community=="Gogo","reef","freshwater"))
mg$degsFromEquator = "?"
...
for (i in which(numeric_columns)) {
  for (j in which(numeric_columns)) {
    if (i != j) {
      plot(mg[, i], mg[, j], col = as.factor(mg$community), pch = 19,
           xlab = colnames(mg)[i], 
           ylab = colnames(mg)[j],
           #method = "overplot",
...
#saveRDS(filepath/Devonian_traits_Miguasha&Gogo_tidy.rds")