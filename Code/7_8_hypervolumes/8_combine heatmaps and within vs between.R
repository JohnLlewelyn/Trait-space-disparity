...
# Distance plots
DTime <- readRDS("filepath/output/dist_time_plot.rds")
DHabitat <- readRDS("filepath/output/dist_habitat_plot.rds")
DClimate <- readRDS("filepath/output/dist_climate_plot.rds")
DCentroids <- readRDS("filepath/output/centroid_dist.rds")

# Overlap plots
JTime <- readRDS("filepath/output/dist_time_plotJ.rds")
JHabitat <- readRDS("filepath/output/dist_habitat_plotJ.rds")
JClimate <- readRDS("filepath/output/dist_climate_plotJ.rds")
jac_plot <- readRDS("filepath/output/jack_plot.rds")
...
ggsave(plot = combined_dist, 
       filename = "filepath/figures/for manuscript/figure 2. combine_dist.png",
...
ggsave(plot = jacs_combined, 
       filename = "filepath/figures/for manuscript/figure 3. combine_JaccInd.png",
       height = 20, width =30,  dpi = 300, device = "png",limitsize = FALSE,
       bg = "white")
...
btwC <- DCentroids$data$Distance[btw_rowsF(DCentroids)]
wiC <- DCentroids$data$Distance[!btw_rowsF(DCentroids)]
lapply(list(btwC,wiC),FUN=mean_sd)

# Now Jaccard distances
names(jac_plot$data) <- c("Row", "Column", "jaccard")
btwJ <- jac_plot$data$jaccard[btw_rowsF(jac_plot)]
wiJ <- jac_plot$data$jaccard[!btw_rowsF(jac_plot)]
