...
bandwidths_fixed <- estimate_bandwidth(hvsprep[[1]], method="fixed", value=mxs)
hvs <- hypervolume_gaussian(hvsprep[[1]], kde.bandwidth=bandwidths_fixed)

set.seed(123)
start_time <- Sys.time()
hvsprep <- lapply(site_data_frames1, function(x){x <-x[,1:8]})
bandwidths_fixed <- lapply(hvsprep, function(x) {x <-estimate_bandwidth(x,method="fixed", value=mxs)})
hvs <- list()
...
saveRDS(dist_time_plot, "Data/data_for_plots/dist_time_plotMAX.rds")
saveRDS(vols, "Data/data_for_plots/volsMAX.rds")
...
saveRDS(dist_habitat_plot, "Data/data_for_plots/dist_habitat_plotMAX.rds")
...
saveRDS(dist_climate_plot, "Data/data_for_plots/dist_climate_plotMAX.rds")
...
saveRDS(centroid_dist, "Data/data_for_plots/centroid_distMAX.rds")
...
saveRDS(overlap_stats_list,"Data/data_for_plots/overlap_stats_listMAX.RDS")
...
saveRDS(jac_long, "Data/data_for_plots/jac_longMAX.rds")
...
saveRDS(dist_time_plotJ, "Data/data_for_plots/dist_time_plotJMAX.rds")
...
saveRDS(dist_habitat_plotJ, "Data/data_for_plots/dist_habitat_plotJMAX.rds")
...
saveRDS(dist_climate_plotJ, "Data/data_for_plots/dist_climate_plotJMAX.rds")
...
saveRDS(jac_plot, "Data/data_for_plots/jack_plotMAX.rds")
...
#check it
plot(jdf$BJtu+jdf$BJne,jdf$jaccard) #looks good, but is the inverse. 
# jdf$BJne <- jdf$jac - jdf$BJne
# plot(jdf$BJtu+jdf$BJne,jdf$jac)

saveRDS(jdf, "GitHub_code&data_revised/Data/jdf_maxBWMAX.rds")
#now make the plots
