...
#Use first 3 PCoAs to build hypervolumes, get the average bandwidths for each axis and then set them as fixed bandwidths (so the same are used for each assemblage)
#PCoA axes are roughly normally distributed, so use Guassian method
set.seed(123) 
start_time <- Sys.time()
hvsprep <- lapply(site_data_frames1, function(x){x <-x[,1:3]})
...
bandw <- data.frame(site=character(), bw1=numeric(),bw2=numeric(),bw3=numeric(), stringsAsFactors=FALSE)
# Loop through each hypervolume in hvs
for(i in 1:length(hvs1)) {
  # Append a new row to rdf
...
end_time <- Sys.time()
end_time - start_time #takes 9 secs, much quicker with fewer PCoAs
...
saveRDS(dist_time_plot, "Data/data_for_plots/dist_time_plot3PCoA.rds")
saveRDS(vols, "Data/data_for_plots/vols3PCoA.rds")
...
saveRDS(dist_habitat_plot, "Data/data_for_plots/dist_habitat_plot3PCoA.rds")
...
saveRDS(dist_climate_plot, "Data/data_for_plots/dist_climate_plot3PCoA.rds")
...
saveRDS(centroid_dist, "Data/data_for_plots/centroid_dist3PCoA.rds")

pdf("Data/data_for_plots/centroid_distances_heatmap3PCoA.pdf", width = 12, height = 11)
centroid_dist
dev.off()
...
saveRDS(overlap_stats_list,"Data/data_for_plots/overlap_stats_list3PCoA.RDS")
...
saveRDS(jac_long, "Data/data_for_plots/jac_long3PCoA.rds")
...
saveRDS(dist_time_plotJ, "Data/data_for_plots/dist_time_plotJ3PCoA.rds")
...
saveRDS(dist_habitat_plotJ, "Data/data_for_plots/dist_habitat_plotJ3PCoA.rds")
...
saveRDS(dist_climate_plotJ, "Data/data_for_plots/dist_climate_plotJ3PCoA.rds")
...
saveRDS(jac_plot, "Data/data_for_plots/jack_plot3PCoA.rds")

pdf("Data/data_for_plots/Jaccard_overlap_heatmap_flip3PCoA.pdf", width = 12, height = 11)
jac_plot
dev.off()
...
#check it
plot(jdf$BJtu+jdf$BJne,jdf$jaccard) #looks good, but is the inverse. Fix it so it's the right way around (like Jaccard)
# jdf$BJtu <- jdf$jaccard - jdf$BJtu
# jdf$BJne <- jdf$jaccard - jdf$BJne
# plot(jdf$BJtu+jdf$BJne,jdf$jaccard)

saveRDS(jdf, "GitHub_code&data_revised/Data/jdf_meanBW3PCoA.rds")
#now make the plots
