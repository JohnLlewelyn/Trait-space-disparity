#build hypervolumes and get metrics - even with only 3 PCoAs, it hangs
library(mFD)
library(dplyr)
library(tidyr)
library(gawdis)
...
#Use convex hulls, so volumes are comparable to those from mFD
set.seed(123) 
start_time <- Sys.time()
hvsprep <- lapply(site_data_frames1, function(x){x <-x[,1:3]})
hvs1 <- lapply(hvsprep, expectation_convex, check.memory=F)
end_time <- Sys.time()
end_time - start_time #
...
saveRDS(dist_time_plot, "Data/data_for_plots/dist_time_plotConvexHull.rds")
saveRDS(vols, "Data/data_for_plots/volsConvexHull.rds")
...
saveRDS(dist_habitat_plot, "Data/data_for_plots/dist_habitat_plotConvexHull.rds")
...
saveRDS(dist_climate_plot, "Data/data_for_plots/dist_climate_plotConvexHull.rds")
...
saveRDS(centroid_dist, "Data/data_for_plots/centroid_distConvexHull.rds")

pdf("Data/data_for_plots/centroid_distances_heatmapConvexHull.pdf", width = 12, height = 11)
centroid_dist
dev.off()
...
saveRDS(overlap_stats_list,"Data/data_for_plots/overlap_stats_listConvexHull.RDS")
...
saveRDS(jac_long, "Data/data_for_plots/jac_longConvexHull.rds")
...
saveRDS(dist_time_plotJ, "Data/data_for_plots/dist_time_plotJConvexHull.rds")
...
saveRDS(dist_habitat_plotJ, "Data/data_for_plots/dist_habitat_plotJConvexHull.rds")
...
saveRDS(dist_climate_plotJ, "Data/data_for_plots/dist_climate_plotJConvexHull.rds")
...
saveRDS(jac_plot, "Data/data_for_plots/jack_plotConvexHull.rds")

pdf("Data/data_for_plots/Jaccard_overlap_heatmap_flipConvexHull.pdf", width = 12, height = 11)
jac_plot
dev.off()
...
saveRDS(jdf, "GitHub_code&data_revised/Data/jdf_ConvexHullW.rds")
#now make the plots
