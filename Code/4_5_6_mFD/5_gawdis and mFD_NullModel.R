...
new_env <- new.env()
source("code/2_3_tidy data/2_combine_and_tidy_modern_RDS files.R", local = new_env)
...
new_env <- new.env()
source("code/2_3_tidy data/3_Devonian_tidy.R", local = new_env)
...
################################################################################
#observed values
obs_alpha <- alpha.fd.multidim(sp_faxes_coord = sp_coords1[,paste("PC",1:7, sep="")], asb_sp_w = as.matrix(presence_matrix))
...
# null_fd <- readRDS(paste(filepath,"/output/null_results.RDS", sep=""))

#Calculate means and SDs across lists e.g., mean and SD for each communities Fric
#mean
...
filepath <- "filepath/"
saveRDS(obs_alpha, paste(filepath,"/output/Plot_orig.RDS", sep = ""))
...
#all traits figure
pdf("filepath/figures/metrics_all_traits_SES.pdf",width=12, height=10)
metricsPlots1
dev.off()
...
# 
pdf("filepath/figures/5metrics_all_traits_SES.pdf",width=12, height=10)
Plot1
dev.off()

saveRDS(Plot1,"output/Plot1_SES.RDS")
