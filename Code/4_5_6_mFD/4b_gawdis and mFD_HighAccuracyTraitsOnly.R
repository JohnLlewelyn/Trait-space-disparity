...
#cut to high accuracy traits only
filepath <- "filepath/"
data_cut <- readRDS(paste(filepath,"/output/DevCut_2025.RDS", sep=""))
names(data_cut)

traits <- traits[,c("logTL","HL","BodyShapeII","PosofMouth","eye.position","spiracle","caudal.fin.shape")]
rm(trait, fish, nums)
##get distances using gawdis###################################################################
GD_all <- gawdis(traits) #no issue with negative weights, but have unbalanced distributions involving some traits - by default, gawdis controls so contributions are equal 
attr(GD_all,"correls") #contribution are equal
attr(GD_all,"weights") #all have positive weights

qual1 <- quality.fspaces(sp_dist = GD_all, fdendro = "average",maxdim_pcoa = 7,deviation_weighting = c("absolute", "squared"),fdist_scaling = c(TRUE, FALSE)) 
...
round(qual1$quality_fspaces[1:7, ], 4) 
...
print(cumsum(eigs$perc)[4]) #4 eigs  explain 74%
print(cumsum(eigs$perc)[5]) #5 eigs explain 85%  
print(cumsum(eigs$perc)[6]) #6 eigs explains 93% 
print(cumsum(eigs$perc)[7]) #7 eigs explains 100% - use all 7
...
#all traits figure
pdf(paste(filepath,"/figures/metrics_all_traitsEig5HIGHAccuracyTraits.pdf",sep=""),width=18, height=15)
metricsPlots3
dev.off()
...
#and plot them
pdf(paste(filepath,"/figures/5metrics_all_traitsHighAccuracyTraits.pdf",sep=""),width=12, height=10)
Plot3
dev.off()

saveRDS(Plot3,paste(filepath,"/output/Plot3HighAccuracyTraits.RDS",sep=""))
