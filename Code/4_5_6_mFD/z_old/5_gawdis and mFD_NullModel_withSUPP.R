...
####SHUFFLING APPEARS TO WORK - Modify code below, include more metrics
...
null_fd <- readRDS("Data/null_results.RDS")
...
wkD <- "~/Dropbox/Devonian fish/manuscript/ProcB revision/GitHub_code&data_revised"
saveRDS(obs_alpha, paste(wkD,"/data/data_for_plots/Plot_orig.RDS", sep = ""))
...
#all traits figure
pdf("~/Dropbox/Devonian fish/manuscript/ProcB revision/figures/metrics_all_traits_SES.pdf",width=12, height=10)
metricsPlots1
dev.off()

#in case it crashes, because it takes a while to get here
save(list = ls(all.names = TRUE), file = "Data/my_workspace_compNOunbalanced_SES.RData")
...
# 
pdf("~/Dropbox/Devonian fish/manuscript/ProcB revision/figures/5metrics_all_traits_SES.pdf",width=12, height=10)
Plot1
dev.off()

saveRDS(Plot1,"Data/data_for_plots/Plot1_SES.RDS")

# #combine the combos in a single plot
# pdf("~/Dropbox/Devonian fish/manuscript/ProcB revision/figures/figure S3. 3metrics_traitCombos_controlDiversity10.pdf",width=18, height=10)
