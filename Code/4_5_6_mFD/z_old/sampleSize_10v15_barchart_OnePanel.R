...
Plot10 <- readRDS("Data/data_for_plots/Plot1_subsample10.RDS")
Plot15 <- readRDS("Data/data_for_plots/Plot1_subsample15.RDS")

#cut to two metrics and make Devonian black so they stand out, tropical with a black outline
d1 <- Plot15$data
d3 <- Plot10$data
d1 <- d1[d1$metric%in%c("richness","nearest\nneighbour"),]
d3 <- d3[d3$metric%in%c("richness","nearest\nneighbour"),]
...
nn2 <- plot_barchart(df2,"nearest neighbour") + theme(legend.position = "none", axis.title.y=element_blank(),axis.text.x = element_text(angle = 45,  vjust = 1, hjust = 1))
...
row_heights <- c(0.25,2.4,3.1)  # Adjust the heights
#add panel labels
...
bp1 <- ggplot() + 
  annotate("text", label = "10 sampled", x = 0.5, y = 0.6, angle = 0, size = 14) +
  xlim(0, 1) + 
  ylim(0.575, 0.612) +
  theme_void() +
...
bp2 <- ggplot() + 
  annotate("text", label = "15 sampled", x = 0.5, y = 0.6, angle = 0, size = 14) +
  xlim(0, 1) + 
  ylim(0.575, 0.612) +
  theme_void() +
  theme(plot.margin = margin(0,0,0,0))
...
ggsave(plot = combined_plots, 
       filename = "~/Dropbox/Devonian fish/manuscript/ProcB revision/figures/sample size comparison_Two FD metrics all species and control for diversity10.png",
