...
# Function to sample individuals
sample_individuals <- function(presence_matrix, sample_size) {
  pm <- presence_matrix[row.names(presence_matrix) != "traits_Little_Rock_Lake", ]
  for (j in 1:nrow(pm)) {
    ones_indices <- which(pm[j, ] == 1)
    if (length(ones_indices) > sample_size) {
      shuffled_indices <- sample(ones_indices)
      switch_indices <- shuffled_indices[(sample_size + 1):length(shuffled_indices)]
      pm[j, switch_indices] <- 0
    }
  }
  # Keep only columns with at least one presence
  kp <- which(colSums(pm) > 0)
  pm <- pm[, kp]
  return(pm)
}

# Create results for different sample sizes
res10 <- list()
res15 <- list()

for (i in 1:100) {
  res10[[i]] <- sample_individuals(presence_matrix, 10)
  res15[[i]] <- sample_individuals(presence_matrix, 15)
}
...
start_time <- Sys.time()
# Create a list of the result sets
res_list <- list(res10 = res10, res15 = res15)

# Create a nested list to store results for each res5, res10, and res15
sct <- list()

# Outer loop to iterate over each result set (res5, res10, res15)
for (res_name in names(res_list)) {
  sct[[res_name]] <- list() # Initialize a list for this result set
  
  for (i in 1:3) {
    sc <- sp_coords[[i]]
    sct[[res_name]][[i]] <- lapply(res_list[[res_name]], function(x) {
      alpha.fd.multidim(
        sp_faxes_coord = sc[, paste("PC", 1:7, sep = "")],
        asb_sp_w = as.matrix(x)
      )
    })
  }
}
...
saveRDS(sct, "Data/sct_sub_sampled metrics.RDS")

start_time <- Sys.time()
alpha_FD10 <- lapply(res10, appA)
alpha_FD15 <- lapply(res15, appA)
end_time <- Sys.time()
end_time - start_time #takes a bit of time (~28 minutes)
saveRDS(alpha_FD10, "Data/sub_sampled metrics10.RDS")
saveRDS(alpha_FD15, "Data/sub_sampled metrics10.RDS")

#functional diversity according to different metrics
sct10 <- sct$res10
sct15 <- sct$res15
indsc10 <- lapply(sct10, function(outer_list) {
  lapply(outer_list, function(inner_list) {
    inner_list$functional_diversity_indices
  })
})

indsc15 <- lapply(sct15, function(outer_list) {
  lapply(outer_list, function(inner_list) {
    inner_list$functional_diversity_indices
  })
})

inds10 <- lapply(alpha_FD10, function(x) x$functional_diversity_indices)
inds15 <- lapply(alpha_FD15, function(x) x$functional_diversity_indices)

#and details of distribution
dets10 <- lapply(sct10, function(outer_list) {
  lapply(outer_list, function(inner_list) {
    inner_list$details
  })
})

dets15 <- lapply(sct15, function(outer_list) {
  lapply(outer_list, function(inner_list) {
    inner_list$details
  })
})

detsA10 <- lapply(alpha_FD10, function(x) x$details)
detsA15 <- lapply(alpha_FD15, function(x) x$details)

indsct10 <- lapply(indsc10, function(outer_list) {
