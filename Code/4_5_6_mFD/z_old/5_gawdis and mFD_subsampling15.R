...
# loop to sub-sample 15 species from each assemblage and save as a list
set.seed(89)
res <- list()
for(i in 1:100) {
  print(i)
  ...
  for (j in 1:nrow(pm)) {
    ...
    if (length(ones_indices) > 15) {
      shuffled_indices <- sample(ones_indices)
      switch_indices <- shuffled_indices[16:length(shuffled_indices)]
...
saveRDS(sct, "Data/sct_sub_sampled15_metrics.RDS")

start_time <- Sys.time()
alpha_FD1 <- lapply(res, appA)
end_time <- Sys.time()
end_time - start_time #takes a bit of time
saveRDS(alpha_FD1, "Data/sub_sampled15_metrics.RDS")
...
#all traits figure
pdf("~/Dropbox/Devonian fish/manuscript/ProcB revision/figures/metrics_all_traits_controlDiversity15.pdf",width=12, height=10)
metricsPlots1
dev.off()

#in case it crashes, because it takes a while to get here
save(list = ls(all.names = TRUE), file = "Data/my_workspace_compNOunbalanced_subsampling15.RData")
...
pdf("~/Dropbox/Devonian fish/manuscript/ProcB revision/figures/3metrics_all_traits_controlDiversity15.pdf",width=12, height=10)
Plot1
dev.off()

saveRDS(Plot1,"Data/data_for_plots/Plot1_subsample15.RDS")

#combine the combos in a single plot
pdf("~/Dropbox/Devonian fish/manuscript/ProcB revision/figures/figure S3. 3metrics_traitCombos_controlDiversity15.pdf",width=18, height=10)
