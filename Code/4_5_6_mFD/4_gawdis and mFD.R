...
# Save all the trait data, including with imputations
write.csv(traits, "filepath/supplementary material/Tables/Table S4 Observed and inferred traits.csv")

##get distances using gawdis###################################################################
GD_all <- gawdis(traits) #no issue with negative weights, but have unbalanced distributions involving some traits - default keeps contributions equal
attr(GD_all,"correls") #contribution are equal
attr(GD_all,"weights") #all have positive weights

qual1 <- quality.fspaces(sp_dist = GD_all, fdendro = "average",maxdim_pcoa = 11,deviation_weighting = c("absolute", "squared"),fdist_scaling = c(TRUE, FALSE)) 
...
round(qual1$quality_fspaces[1:15, ], 4) 
...
print(cumsum(eigs$perc)[6]) #6 eigs  explain 78%
print(cumsum(eigs$perc)[7]) #7 eigs explain 84% when all traits are included - go with 7 because smallest community has only 8 species
...
Tcorrs1 <- traits.faxes.cor(sp_tr = traits, sp_faxes_coord = sp_coords1[,paste("PC",1:7, sep="")])
Tcorrs1 <- Tcorrs1[order(Tcorrs1$axis, Tcorrs1$value), ]
#save it
filepath <- "filepath/"
write.csv(Tcorrs1,paste(filepath,"/tables/PC_trait_correls.csv", sep=""), row.names = FALSE)
...
alpha_FD3 <- alpha.fd.multidim(sp_faxes_coord = sp_coords1[,paste("PC",1:7, sep="")], asb_sp_w = as.matrix(presence_matrix))
...
#all traits figure
pdf(paste(filepath,"/figures/metrics_all_traitsEig5.pdf",sep=""),width=18, height=15)
metricsPlots3
dev.off()
...
#and plot them
pdf(paste(filepath,"/figures/5metrics_all_traits.pdf",sep=""),width=12, height=10)
Plot3
dev.off()

saveRDS(Plot3,paste(filepath,"/output/Plot3.RDS",sep=""))
...
centroids3 <- cr(dets3,9)

#save as table
cent3_round <- centroids3
cent3_round <- cent3_round %>%
  mutate_if(is.numeric, round, digits = 3)
write.csv(cent3_round,"filepath/supplementary material/PC_trait_centroids.csv", row.names = FALSE)
