...
Plot3 <- readRDS("filepath/output/Plot3morphometrics.RDS")
d3 <- Plot3$data
...
eve <- plot_barchart(df,"evenness") + theme(legend.position = "none", axis.text.x = element_text(angle = 45,  vjust = 1, hjust = 1)) + 
  scale_y_continuous(breaks = c(0.0, 0.2, 0.4, 0.6, 0.8),labels = scales::number_format(accuracy = 0.1), expand = expansion(mult = c(0, 0)))
...
ggsave(plot = combined_plots, 
       filename = "filepath/supplementary material/Sfigure #. Three FD metrics all species Morphometrics.png",
